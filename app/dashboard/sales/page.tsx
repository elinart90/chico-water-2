...
export default function SalesDashboard() {
  ...
  useEffect(() => {
    fetch('/api/orders')
      .then(r => r.json())
      .then(d => setOrders(d.orders || MOCK_ORDERS))
        ...
        <nav className="flex-1 p-3 space-y-1">
          ...
          <button
            ...
            className={cn(
              ...
              tab === 'orders' ? 'bg-white/15 text-white' : 'text-white/60 hover:text-white hover:bg-white/10'
            )}>
            <ShoppingBag className="w-4 h-4 shrink-0" />
            <span className="flex-1 text-left">My Orders</span>
            {pending > 0 && (
              <span className="bg-red-500 text-white text-[10px] font-bold px-1.5 py-0.5 rounded-full">{pending}</span>
            )}
          </button>
          ...
          <button
            ...
            className={cn(
              'w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all',
              tab === 'requests' ? 'bg-white/15 text-white' : 'text-white/60 hover:text-white hover:bg-white/10'
            )}>
          ...
          {/* New PR link */}
          <Link
            href="/dashboard/sales/purchase-request"
            ...
            <Plus className="w-4 h-4 shrink-0" /> New PR
          </Link>

          <Link
            href="/"
            className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium text-white/60 hover:text-white hover:bg-white/10">
            <Droplets className="w-4 h-4 shrink-0" /> View Website
          </Link>
        </nav>
      ...
      <main className="ml-56 flex-1 p-6">
          ...
          <>
              ...
              <div className="flex items-center gap-3">
                {pending > 0 && (
                  <div className="relative">
                    ...
                    <span className="absolute -top-1 -right-1 w-4 h-4 bg-red-500 rounded-full text-white text-[10px] flex items-center justify-center font-bold">{pending}</span>
                  </div>
                )}
            ...
            <div className="grid grid-cols-3 gap-4 mb-6">
              {[
                ...
                { label: 'Revenue', value: formatCurrency(totalRevenue), icon: ShoppingBag, color: 'text-green-600', bg: 'bg-green-50' },
              ].map(stat => (
              ...
              {(['all', 'pending', 'confirmed', 'packed', 'in_transit', 'delivered'] as const).map(f => (
                <button key={f} onClick={() => setFilter(f)}
                  className={cn('px-3 py-1.5 rounded-xl text-xs font-semibold capitalize transition-all',
                    filter === f ? 'bg-water-600 text-white' : 'bg-white text-gray-500 border border-gray-200 hover:border-gray-300')}>
                  {f === 'all' ? 'All' : ORDER_STATUS_LABELS[f]}
                  {f === 'pending' && pending > 0 && (
                    <span className="ml-1 bg-red-500 text-white rounded-full px-1.5 text-[10px]">{pending}</span>
                  )}
                </button>
              ))}
            </div>
              ...
              <div className="space-y-4">
                {filtered.map(order => (
                  <div key={order.id} className={cn(
                    'bg-white rounded-2xl border-2 shadow-sm p-5',
                    ...
                    <div className="flex items-start justify-between flex-wrap gap-3 mb-4">
                      <div>
                        <div className="flex items-center gap-3 mb-1 flex-wrap">
                          ...
                          <span className={cn('px-2.5 py-1 rounded-full text-xs font-bold', ORDER_STATUS_COLORS[order.status])}>
                        ...
                        {order.delivery_notes && (
                          <p className="text-xs text-amber-600 mt-1 font-medium">Note: {order.delivery_notes}</p>
                        )}
                      </div>
                    ...
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      {order.status === 'confirmed' && (
                        <button onClick={() => updateStatus(order.id, 'packed')} disabled={updating === order.id}
                          className="flex items-center gap-1.5 bg-purple-600 hover:bg-purple-700 text-white text-xs font-bold px-4 py-2 rounded-xl disabled:opacity-50">
                          <Package className="w-3.5 h-3.5" /> Mark as Packed
                        </button>
                      )}
                      {order.status === 'packed' && (
                        <button onClick={() => updateStatus(order.id, 'in_transit')} disabled={updating === order.id}
                          className="flex items-center gap-1.5 bg-blue-600 hover:bg-blue-700 text-white text-xs font-bold px-4 py-2 rounded-xl disabled:opacity-50">
                          <Truck className="w-3.5 h-3.5" /> Dispatch
                        </button>
                      )}
                      {order.status === 'in_transit' && (
                        <button onClick={() => updateStatus(order.id, 'delivered')} disabled={updating === order.id}
                          className="flex items-center gap-1.5 bg-green-600 hover:bg-green-700 text-white text-xs font-bold px-4 py-2 rounded-xl disabled:opacity-50">
        ...
        {tab === 'requests' && (
          <>
            <div className="flex items-center justify-between mb-6">
              <div>
                <h1 className="text-xl font-black text-gray-900">My Purchase Requests</h1>
                <p className="text-gray-500 text-sm">Track the status of your submitted requests</p>
              </div>
              <Link href="/dashboard/sales/purchase-request"
                className="flex items-center gap-2 bg-water-600 hover:bg-water-700 text-white text-sm font-semibold px-4 py-2.5 rounded-xl transition-colors">
              ...
              <div className="bg-red-50 border border-red-200 rounded-2xl p-4 mb-6 flex items-start gap-3">
                ...
                <div>
                  <p className="font-bold text-red-800 text-sm">
                    {rejectedCount} purchase request{rejectedCount !== 1 ? 's were' : ' was'} rejected
                  </p>
            ...
            {prLoading ? (
              ...
              <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-12 text-center">
                ...
                <Link href="/dashboard/sales/purchase-request" className="btn-primary inline-flex items-center gap-2">
              ...
              <div className="space-y-4">
                {prs.map(pr => (
                  <div key={pr.id} className={cn(
                    'bg-white rounded-2xl border-2 shadow-sm p-5',
                    pr.status === 'rejected' ? 'border-red-200' :
                    pr.status === 'approved' ? 'border-green-200' :
                    pr.status === 'pending' ? 'border-yellow-200' : 'border-gray-100'
                  )}>
                    <div className="flex items-start justify-between flex-wrap gap-3 mb-4">
                      <div>
                        <div className="flex items-center gap-2 mb-1 flex-wrap">
                          <span className="font-mono font-black text-water-600 text-sm">{pr.request_number}</span>
                          <span className={cn('px-2.5 py-1 rounded-full text-xs font-bold capitalize', PR_STATUS_COLORS[pr.status])}>
                            {pr.status}
                          </span>
                          <span className={cn('px-2 py-0.5 rounded-full text-xs font-semibold capitalize', URGENCY_COLORS[pr.urgency])}>
                            {pr.urgency}
                          </span>
                        </div>
                        {pr.supplier_name && <p className="text-sm text-gray-500">Supplier: {pr.supplier_name}</p>}
                        <p className="text-xs text-gray-400 mt-0.5">
                          Submitted {new Date(pr.created_at).toLocaleDateString('en-GH', { day: 'numeric', month: 'short', year: 'numeric' })}
                        </p>
                      </div>
                    ...
                    {pr.items?.length > 0 && (
                      <div className="bg-gray-50 rounded-xl p-3 mb-3">
                        <p className="text-xs text-gray-500 font-medium mb-2">Items requested</p>
                        {pr.items.map((item, i) => (
                          <div key={i} className="flex justify-between text-sm py-1">
                      ...
                      <div className="bg-green-50 border border-green-200 rounded-xl p-3 flex items-start gap-2">
                        ...
                        <div>
                          <p className="text-xs font-bold text-green-800">
                            Approved by {pr.approver_name || 'Admin'}
                            {pr.approved_at && ` on ${new Date(pr.approved_at).toLocaleDateString('en-GH', { day: 'numeric', month: 'short' })}`}
                          </p>
                      ...
                      <div className="bg-red-50 border border-red-200 rounded-xl p-3 flex items-start gap-2">
                        ...
                        <div>
                          <p className="text-xs font-bold text-red-800">
                            Rejected by {pr.approver_name || 'Admin'}
                            {pr.approved_at && ` on ${new Date(pr.approved_at).toLocaleDateString('en-GH', { day: 'numeric', month: 'short' })}`}
                          </p>
                          {pr.approval_note && (
                            <p className="text-xs text-red-700 mt-1 bg-red-100 rounded-lg px-2 py-1">
                              Reason: {pr.approval_note}
                            </p>
                          )}
                          <Link href="/dashboard/sales/purchase-request"
                            className="inline-flex items-center gap-1 text-xs text-red-700 font-semibold mt-2 hover:underline">
                            <Plus className="w-3 h-3" /> Submit revised request
                          </Link>
                        </div>
                      </div>
                    )}
                    ...
                    {pr.status === 'pending' && (
                      <div className="bg-yellow-50 border border-yellow-200 rounded-xl p-3 flex items-center gap-2">
                        ...
                        <p className="text-xs text-yellow-800 font-medium">
                          Waiting for admin approval. You will see the decision here once reviewed.
                        </p>
                      </div>
                    )}
