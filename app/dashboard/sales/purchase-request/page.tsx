...
export default function NewPurchaseRequestPage() {
  ...
  useEffect(() => {
    supabase.from('suppliers').select('id, name, category').eq('is_active', true).order('name').then(({ data }) => setSuppliers(data || []))
    supabase.from('purchase_requests').select('*').order('created_at', { ascending: false }).limit(10).then(({ data }) => setMyRequests(data || []))
  }, [])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const res = await fetch('/api/purchase-requests', {
      ...
      body: JSON.stringify({
        ...
        items: items.map(i => ({ name: i.name, quantity: parseFloat(i.quantity), unit: i.unit, unit_cost: parseFloat(i.unit_cost) || 0, total_cost: i.total_cost })),
    ...
    if (res.ok) {
      ...
      toast.success(`Request ${data.request_number} submitted — waiting for admin approval`)
    } else {
  ...
  if (submitted) return (
    <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-3xl border border-gray-100 shadow-sm p-10 max-w-md w-full text-center">
        ...
        <p className="text-gray-500 mb-4">Your purchase request has been sent to the admin for approval.</p>
        <div className="bg-water-50 rounded-2xl p-4 mb-8">
          <p className="text-sm text-gray-500 mb-1">Request number</p>
          <p className="text-2xl font-black text-water-600">{submitted}</p>
        </div>
        <div className="space-y-3">
          <button onClick={() => { setSubmitted(null); setItems([{ ...EMPTY_ITEM }]); setForm({ supplier_id: '', supplier_name: '', reason: '', urgency: 'normal' }) }}
            className="btn-primary w-full">Submit Another Request</button>
          <Link href="/dashboard/sales" className="btn-secondary w-full block text-center">Back to Dashboard</Link>
        </div>
      </div>
    </div>
  )
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <select value={form.urgency} onChange={e => setForm(f => ({ ...f, urgency: e.target.value }))} className="input bg-white">
              ...
              <div className="sm:col-span-2">
                ...
                <textarea value={form.reason} onChange={e => setForm(f => ({ ...f, reason: e.target.value }))}
                  ...
                  placeholder="Explain why this purchase is needed (e.g. sachet water bags running out, need restock for next week's orders)" required />
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
            <div className="flex items-center justify-between mb-5">
              ...
              <button type="button" onClick={addItem} className="flex items-center gap-1.5 text-water-600 text-sm font-semibold hover:text-water-700">
              ...
              {items.map((item, i) => (
                <div key={i} className="grid grid-cols-12 gap-3 items-start">
                  <div className="col-span-4">
                    ...
                    <input value={item.name} onChange={e => updateItem(i, 'name', e.target.value)} className="input" placeholder="e.g. 500ml PET Bottles" required />
                  </div>
                  <div className="col-span-2">
                    {i === 0 && <label className="label">Qty *</label>}
                    <input type="number" value={item.quantity} onChange={e => updateItem(i, 'quantity', e.target.value)} className="input" placeholder="0" min="1" required />
                  </div>
                  <div className="col-span-2">
                    {i === 0 && <label className="label">Unit</label>}
                    <input value={item.unit} onChange={e => updateItem(i, 'unit', e.target.value)} className="input" placeholder="bags" />
                  </div>
                  <div className="col-span-2">
                    {i === 0 && <label className="label">Unit Cost</label>}
                    <input type="number" value={item.unit_cost} onChange={e => updateItem(i, 'unit_cost', e.target.value)} className="input" placeholder="0.00" step="0.01" />
                  </div>
                  <div className="col-span-1">
                    {i === 0 && <label className="label">Total</label>}
                    <div className="h-[46px] flex items-center text-sm font-bold text-gray-900">{item.total_cost > 0 ? formatCurrency(item.total_cost) : '—'}</div>
                  </div>
          ...
          <button type="submit" disabled={saving}
            ...
            {saving ? <><Loader className="w-5 h-5 animate-spin" /> Submitting...</> : <><Send className="w-5 h-5" /> Submit for Admin Approval</>}
          </button>
          <p className="text-center text-xs text-gray-400">
            This request will be reviewed by the admin before any purchase is made. You will be notified of the decision.
          </p>
        </form>
      </div>
    </div>
